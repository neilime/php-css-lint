...
class Tokenizer
{
    ...
    private function processCharacter(string $char): Generator
    {
        ...
        foreach ($this->getParsers() as $parser) {
            ...
            foreach ($this->parseCharacter($parser) as $result) {
                ...
                $shouldYieldResult = $result instanceof LintError || $result->getParent() === null;
            ...
            // Token has been generated, reset current content except if we are parsing a block
            $shouldResetContent = $generatedToken || $isBlockStart;
            if ($shouldResetContent) {
                $this->tokenizerContext->resetCurrentContent();
            }

            // Token has been generated, set current token except if token has parent
            ...
            $generatedTokenParent = $generatedToken ? $generatedToken->getParent() : null;
            if ($generatedTokenParent) {
                $this->tokenizerContext->setCurrentToken($generatedTokenParent);
            }
    ...
    private function assertBlockTokenIsClean(BlockToken $blockToken): Generator
    {
        ...
        foreach ($nonCompleteTokens as $nonCompleteToken) {
            ...
            yield new TokenError(
                LintErrorKey::UNCLOSED_TOKEN,
                sprintf('Unclosed "%s" detected', $nonCompleteToken->getType()),
                $nonCompleteToken,
            );
        }
