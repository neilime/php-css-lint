...
class Linter
{
    ...
    public function getTokenizer(): Tokenizer
    {
        ...
        return $this->tokenizer = new Tokenizer($this->getLintConfiguration());
    }
    ...
    private function lintTokens(iterable $tokens): Generator
    {
        ...
        foreach ($tokens as $tokenOrError) {
            ...
            foreach ($linters as $tokenLinter) {
                if (!$tokenLinter->supports($tokenOrError)) {
                    continue;
                }

                yield from $tokenLinter->lint($tokenOrError);
            }

            $tokenValue = $tokenOrError->getValue();
