...
class Cli
{
    ...
    private function assertOptionsAreLintConfiguration(mixed $options): void
    {
        ...
        if (isset($options['allowedIndentationChars']) && !is_array($options['allowedIndentationChars'])) {
            throw new RuntimeException('Invalid option key: allowedIndentationChars');
        }

        // Assert that the constructors is an array of strings
        if (isset($options['constructors']) && !is_array($options['constructors'])) {
            throw new RuntimeException('Invalid option key: constructors');
        }

        // Assert that the standards is an array of strings
        if (isset($options['standards']) && !is_array($options['standards'])) {
            throw new RuntimeException('Invalid option key: standards');
        }

        // Assert that the nonStandards is an array of strings
        if (isset($options['nonStandards']) && !is_array($options['nonStandards'])) {
            throw new RuntimeException('Invalid option key: nonStandards');
        }
    }
