...
class WhitespaceParserTest extends TestCase
{
    ...
    {
        return [
            'single space' => ["\n f", ' ', new Position(2, 0), new Position(2, 1)],
            'multiple spaces' => ["\n   f", '   ', new Position(2, 0), new Position(2, 1)],
        ];
    }
    ...
    public function testParsesWhitespace(string $content, string $expectedValue, Position $expectedStart, Position $expectedEnd): void
    {
        ...
        $tokenizerContext->getCurrentPosition()->incrementLine();
        $tokens = [];
    ...
    public function testHandlesEndOfLineChars(): void
    {
        // Arrange
        $content = "line1\nline2\r\nline3";
        $tokenizerContext = new TokenizerContext();
        ...
        // Assert
        $this->assertEquals(3, $tokenizerContext->getCurrentPosition()->getLine());
    }
}
