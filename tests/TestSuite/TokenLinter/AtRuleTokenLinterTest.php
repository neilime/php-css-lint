...
class AtRuleTokenLinterTest extends TestCase
{
    ...
    {
        $token = new AtRuleToken('media', 'screen', 1, 0, 10);
        $this->assertTrue($this->linter->supports($token), 'Should support AtRuleToken');

        $nonAtRuleToken = new BlockToken([], 1, 0, 4);
    ...
    {
        // Arrange
        $token = new AtRuleToken($name, $value, 1, 0, strlen($name) + (strlen($value ?? '') + 1));
